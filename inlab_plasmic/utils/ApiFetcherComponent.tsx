...
export const ApiFetcherComponent = forwardRef<ApiActions, PropsType>(
  (props: PropsType, ref) => {
    ...
    const onAxiosSuccess = (response: any): void => {
      logForDev(
        "ApiFetcherComponent: axios request success: " +
          (response.data ? "Data Fetched" : "Data Not Fetched")
      );
      setLoading(false);
      setData(response);
    };
    ...
    const onAxiosError = async (error: any) => {
      ...
      logForDev(
        "ApiFetcherComponent: axios request error: " + JSON.stringify(error)
      );
    ...
    useEffect(() => {
      logForDev(
        "ApiFetcherComponent: useEffect: " +
          (inlabUser ? "User found" : "User is NULL")
      );

      const control = new AbortController();
      const searchTimeOut = setTimeout(() => {
        logForDev(
          "ApiFetcherComponent: useEffect: runned: " +
            (inlabUser ? "User found" : "User is NULL")
        );
        // TODO: Handle errors
        inlabUser &&
          refreshAccessIfNeeded(globalContext, inlabUser)
            ...
            .catch((error) => {
              ...
              logForDev(
                "ApiFetcherComponent: useEffect: Refreshing user failed!!! "
              );
              setLoading(false);
            });
      }, props.delay || 0);
