...
class CacheService {
  ...
  /**
   * Gets all patient data for a specific patient
   * @param {string} patientId - The patient identifier (use 'api' for non-patient data)
   * @returns {Promise<object>} An object with cache keys as keys and cached data as values
   */
  async getAllPatientData(patientId) {
    const records = await db.patientCache.where('patientId').equals(patientId).toArray();
    const patientData = {};
    records.forEach((record) => {
      patientData[record.cacheKey] = record.data;
    });
    return patientData;
  }
